...
import type { BrainstormBoard, BrainstormEdge, BrainstormNode, DiaryComment, DiaryData, DiaryEntry, PlannerEvent, StudyGoal, StudyTask, UserPreferences } from './types'
...
interface DiaryState extends DiaryData {
  ...
  saveDailyMemo(date:string,content:string):Promise<void>
  loadSample():Promise<void>; importData(value:unknown):Promise<void>; reset():Promise<void>
}
const snapshot=(state:DiaryState):DiaryData=>({events:state.events,entries:state.entries,comments:state.comments,boards:state.boards,profile:state.profile,calendar:state.calendar,preferences:state.preferences,studyGoals:state.studyGoals,studyTasks:state.studyTasks,dailyMemos:state.dailyMemos})
...
export const newBoard=(name='Untitled board'):BrainstormBoard=>{const n=now();return{id:crypto.randomUUID(),name,nodes:[],edges:[],createdAt:n,updatedAt:n}}
